...
@section('slider')
@include('partials.pageBanner',['bannerContent'=>'Product Cart','bannerImg'=>'page-banner.jpg'])
@endsection
...
<tr>
<td><div class="product-img"><img src='/chnlsgasplant/images/product/{{$productCart->options->image}}' alt="Product"></div></td>
...
<ul>
<li  data-id="{{ $productCart->rowId }}" class="remove-cart"> <a href="#"><i class="fa fa-trash-o"></i></a></li>
</ul>
</div></td>
<td>
    {{$productCart->qty }}
...
<form action="{{ route('cart.destroy') }}" method="POST" style="display: none;" id="clear-cart">
@csrf
@method('DELETE')
...
<p><span> Value Added Tax </span><span>&#8358;{{ $productCart->price * $productCart->qty * 0.75}}</span></p>
...
<a href="{{route('products')}}">NO PRODUCT IN YOUR CART, CLICK HERE TO ADD PRODUCT.</a>
</div>
</div>
</div>
@endif
